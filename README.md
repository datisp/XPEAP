...
## About
The XPEAP (**X P**rime **E**nd **A**nalysis **P**ipeline) allows the detection of RNA 5' or 3'ends in procariotic NGS data. It is written as an easy to use solution which covers every step from initial raw data processing to quantification and the final statistical analysis. The basic idea behind this project is to compare RNA sequencing data from two bacterial strains, each in triplicates, and to identify differential 5'/3'ends. As a microbiologist focused on RNA research you may for example want to compare the presence and abundance of RNA 3'ends in your wildtype strain and an a RNAse deficient strain. If that describes your research questions, XPEAP is the tool you need!
...
`![XPEAP structure](https://raw.githubusercontent.com/datisp/XPEAP/master/structure_simple.svg?token=APCNPXK7Y7UGSVPLE3R3X626RXLSU)`
...
These tools may require individual other software (i.e. READemption uses segemehl as mapper), so please refere to the above mentioned documentations. XPEAP was developed and tested on Ubuntu 16.04.6 LTS (GNU/Linux 4.15.0-74-generic x86_64) with RNA seq data from *Rhodobacter sphaeroides* 2.4.1.
...
**ProTip:** Really make sure that there are no typos in your variables :D 
